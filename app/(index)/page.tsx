...
export default function Home() {
  return (
    <div className="mx-auto max-w-3xl p-0 sm:p-4 md:p-8">
      <article className="space-y-8 text-base sm:text-lg xl:text-xl">
        ...
        <p className="leading-normal">
          ...
          어려운 텍스트 끙끙대며 읽어보기입니다. 필라테스의{' '}
          <Link href="https://youtu.be/9dDGb1LWCv8?si=I2DJLGkuNfLorvPY&t=54">
            Jack Knife
          </Link>{' '}
          동작을 할 수 있는 사람이 되는 게 올해 목표 중 하나입니다. 한때 영화를
          정말 좋아했는데, 오늘은{' '}
          {favoriteMovies[new Date().getDate() % favoriteMovies.length]}를 가장
          좋아한다고 말하고 싶네요. 방탈출을 너무 좋아해서 언젠가 직접 방탈출
          게임 공간을 만들어보고 싶습니다. 프래그머티즘 철학을 좋아하는데, 그
          시초가 된{' '}
      ...
      <nav className="mt-10">
        <ol className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2">
          {allEssays.map((essay) => (
